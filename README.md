...
- **더블클릭** — 그 세션의 창(IntelliJ/VS Code/iTerm2/Terminal, Codex 데스크톱은 해당 작업 화면)을 맨 앞으로. IntelliJ처럼 한 앱에 프로젝트 창이 여럿일 때도 **작업 폴더 이름이 든 정확한 창**을 앞으로 올립니다.
  - 정확한 창 선택은 **손쉬운 사용(Accessibility) 권한**이, iTerm2/Terminal 탭 선택은 **자동화 권한**이 필요합니다. 권한이 없으면 펫이 "손쉬운 사용 권한을 켜주세요" 하고 시스템 설정을 열어 줍니다. **시스템 설정 → 개인정보 보호 및 보안 → 손쉬운 사용**에서 "Claude Session Pets"를 켜 주세요.
  - ⚠️ **직접 빌드해 쓰는 경우**: `build-app.sh`는 서명의 지정 요구사항을 번들 식별자로 고정하므로 재빌드 후에도 권한이 유지됩니다. 이 방식이 적용된 첫 빌드 뒤 한 번만 **시스템 설정 → 손쉬운 사용**에서 앱을 꺼서 다시 켜 주세요(기존 항목은 옛 서명 기준). 로컬 서명 인증서(`./scripts/make-signing-cert.sh`)는 선택 사항입니다.
  - IntelliJ처럼 창 제목에 프로젝트 이름만 있는 경우, 작업 폴더명 → 상위 폴더명 순으로 제목을 찾아 그 창을 올립니다.
...
### 1.2.3
- Claude Code 내부 도우미 프로세스(`claude daemon run`, `bg-pty-host`, `bg-spare`, 작업 폴더 `/tmp/cc-daemon-…/spare`)를 세션으로 오인해 "spare" 펫이 여러 마리 뜨던 문제 수정 — 프로세스 필터에서 제외.
- **IntelliJ/VS Code 정확한 창 앞으로 가져오기가 안 되던 문제** — 손쉬운 사용 권한이 없을 때(-25211) 앱 통째 활성화로 조용히 폴백하던 것을, 권한 확인 → 시스템 프롬프트·설정 열기 → 펫 안내로 바꿨습니다. 창 제목 검색은 작업 폴더명 → 상위 폴더명 순으로 재시도(IntelliJ 모듈 하위 폴더 세션 대응).
- 재빌드마다 권한이 풀리는 원인(ad-hoc 서명 = cdhash 지정 요구사항) 해결: `build-app.sh`가 바깥 번들의 지정 요구사항을 번들 식별자로 고정해 서명(내부는 deep ad-hoc). 적용 후 첫 한 번만 손쉬운 사용 토글 off→on. 로컬 인증서(`scripts/make-signing-cert.sh`)는 선택. 권한 요청 다이얼로그는 1분에 1회, 설정의 손쉬운 사용 화면은 매번 열림.

### 1.2.2
- **유휴 세션에 "작업 완료!"가 반복해서 뜨던 문제 수정.** 트랜스크립트 활동 시각을 파일 mtime으로 재던 탓에, Claude Code가 놀고 있는 세션에 덧붙이는 부기 항목(아티팩트 감시 장부·파일 스냅샷 등)마다 "작업 중→완료"가 다시 떴습니다. 이제 마지막 **메시지** 항목의 시각으로 잽니다.
- 응답 없이 10분 넘게 남은 user 프롬프트(실행되지 않은 대기열 입력)는 살아있는 작업으로 보지 않습니다. 훅 상태가 `working`에 멈춰 있어도 유휴로 표시.
- 상태 훅: 긴 도구 실행 중 발화하는 일반 알림이 "입력 필요"로 덮어쓰던 문제 수정 — 권한 요청/질문(`permission_prompt`·`elicitation_dialog`)만 `waiting`으로 기록. 상태 파일에 진단용 `event`/`ntype` 필드 추가. 앱 시작 시 설치된 훅이 자동 갱신됩니다.

### 1.2.1
- Codex 데스크톱 세션 펫의 모니터 배지가 몸통 위 엉뚱한 자리에 걸치던 문제 수정 — 펫 박스 너비를 도트 스프라이트(72px)에 맞추고 배지를 발 옆 우하단으로, 색은 페리윙클로.
